  ...
  const renderContent = () => {
    ...
    if (activeMenu === 'onecut') {
      ...
      const onecutAll = allRootPosts.filter(p => {
        ...
        if (status && (status.startsWith('exiled_') || status === 'banned')) return false;
        return true;
      });
  ...
  setIsCreateOpen(true);
        ...
        <div className="flex md:hidden items-center gap-1.5 ml-auto shrink-0">
          {userData ? (
            ...
            <button onClick={() => setIsWelcomeOpen(true)} className="flex items-center gap-1 bg-violet-600 text-white px-2 h-5 rounded-lg text-[11px] font-black shadow-sm whitespace-nowrap shrink-0">
