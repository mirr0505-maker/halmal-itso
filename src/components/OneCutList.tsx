              ...
              <div className="pt-1.5 border-t border-slate-50">
                ...
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className="text-slate-400 text-[8px] font-bold truncate">Lv{displayLevel} · {getReputationLabel(authorData ? getReputationScore(authorData) : displayLikes)} · 깐부수 {formatKoreanNumber(realFollowers)}</span>
                  </div>
                </div>
                {/* 통계 줄: 댓글 | 땡스볼 | 좋아요 | 공유 */}
                <div className="flex items-center justify-end gap-2 text-[10px] font-black text-slate-300">
                  ...
                  {(post.thanksballTotal || 0) > 0 && (
                    <span className="flex items-center gap-0.5 text-amber-400">
                      <span className="text-[11px]">⚾</span>{post.thanksballTotal}
                    </span>
                  )}
                  <span
                    onClick={(e) => { e.stopPropagation(); onLikeClick?.(e, post.id); }}
                    className={`flex items-center gap-0.5 cursor-pointer transition-colors ${isLikedByMe ? 'text-rose-500' : 'hover:text-rose-400'}`}
                  >
