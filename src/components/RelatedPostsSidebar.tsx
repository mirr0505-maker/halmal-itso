...
// ⚡ 성능 2026-07-02: 관련글 카드를 React.memo 자식으로 분리해 DOMParser/sanitize를 topic.content 기준 1회만 실행
...
// ⚡ 성능 2026-07-02: getTimeAgo를 모듈 스코프로 올려 자식 memo의 안정적 참조 확보(부모 리렌더 무관)
...
// ⚡ 성능 2026-07-02: 관련글 1건 카드를 React.memo로 분리 — extractFirstImage(DOMParser)·sanitizeHtml·likedBy 스캔을 topic.content 기준 memo화
...
const RelatedPostItem = React.memo(({
  topic, onPostClick, commentCount, currentNickname, allUsers, followerCounts
}: RelatedPostItemProps) => {
  // ⚡ 성능 2026-07-02: extractFirstImage(DOMParser)는 topic.content 변경 시에만 재실행
  const topicImage = useMemo(() => topic.imageUrl || extractFirstImage(topic.content), [topic.imageUrl, topic.content]);
  // ⚡ 성능 2026-07-02: sanitizeHtml도 topic.content 변경 시에만 재실행
  const sanitizedContent = useMemo(() => sanitizeHtml(topic.content), [topic.content]);
  ...
  // ⚡ 성능 2026-07-02: goldStarCount likedBy 전체 스캔 memo화
  const goldStarCount = useMemo(() => (topic.likedBy || []).filter(nick => {
    const ud = allUsers[`nickname_${nick}`];
    return ud && calculateLevel(ud.exp || 0) >= 5;
  }).length, [topic.likedBy, allUsers]);
    ...
    >
      ...
      {topicImage && (
        <div className="w-full aspect-[16/7] rounded-lg overflow-hidden bg-slate-50 border border-slate-50 shrink-0">
          <img src={topicImage} alt="" loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        </div>
      )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-6 h-6 rounded-full bg-slate-50 overflow-hidden shrink-0 border-2 border-white shadow-sm ring-1 ring-slate-100">
              {/* ⚡ 성능 2026-07-02: 사이드바 아바타 지연 로드 */}
              <img src={`https://api.dicebear.com/7.x/adventurer/svg?seed=${topic.author}`} alt="" loading="lazy" className="w-full h-full object-cover" />
            </div>
      ...
      <div className="flex flex-col gap-0 max-h-[calc(100vh-100px)] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pb-20 pl-4 pr-3 border-l-2 border-slate-200">
        ...
        {relatedPosts.map((topic) => (
          <RelatedPostItem
            ...
            commentCount={commentCounts[topic.id] || 0}
