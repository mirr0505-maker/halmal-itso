...
const CreateNakedKing = ({ userData, editingPost, onSubmit, onClose }: Props) => {
  const [postData, setPostData] = useState<Partial<Post>>({
    ...
    factChecked: editingPost?.factChecked ?? false,
    isOneCut: false,
  });
    ...
    <div className="w-full max-w-[860px] mx-auto py-8 animate-in fade-in duration-200">
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col shadow-sm" style={{ maxHeight: 'calc(100vh - 80px)' }}>
        ...
        <div className="flex items-center gap-3 px-5 py-2.5 border-b border-slate-100 shrink-0">
          <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">사실 확인</span>
          <button type="button" onClick={() => setPostData(p => ({ ...p, factChecked: !p.factChecked }))}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all border ${postData.factChecked ? 'bg-emerald-50 text-emerald-700 border-emerald-300' : 'bg-slate-50 text-slate-400 border-slate-200 hover:bg-slate-100'}`}>
            {postData.factChecked ? '⭕ 사실 확인 완료' : '❓ 확인 중 (미완료)'}
          </button>
        </div>
