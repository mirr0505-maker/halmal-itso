        ...
        posts.map((post) => {
          ...
          return (
            <div 
              ...
              className="bg-white border border-slate-100 rounded-[2rem] p-5 cursor-pointer hover:border-blue-400 hover:shadow-xl transition-all group flex flex-col h-full shadow-sm min-h-[380px]"
            >
              {/* 1. 최상단: 제목 및 시간/프로모션 */}
              ...
              {/* 2. 중간: 내용 (3줄 제한) */}
              <p className="text-[13px] text-slate-500 line-clamp-3 mb-4 leading-relaxed font-medium flex-1">
                {stripHtml(post.content)}
              </p>
              
              {/* 3. 하단부: 이미지 (있는 경우) */}
              {post.imageUrl && (
                <div className="w-full aspect-video rounded-2xl overflow-hidden mb-5 shrink-0 bg-slate-50 border border-slate-50">
                  <img src={post.imageUrl} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
              )}
              
              {/* 4. 최하단: 카테고리 & 아바타/유저정보 */}
              <div className="pt-4 border-t border-slate-50 mt-auto flex flex-col gap-3 shrink-0">
