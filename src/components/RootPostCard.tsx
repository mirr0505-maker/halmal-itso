    ...
    <section className="bg-white border border-slate-100 rounded-none mb-0 shadow-sm relative overflow-hidden flex flex-col">
      <div className="flex-1 flex flex-col p-12 md:p-16">
        <div className="flex items-center justify-between mb-6">
          <span className="text-[12px] font-black text-slate-300 uppercase tracking-widest">{formatTime(post.createdAt)}</span>
        ...
        <h2 className="text-[22px] font-[1000] text-slate-900 mb-6 leading-tight tracking-tighter max-w-4xl">{post.title}</h2>

        <div className="flex flex-wrap gap-2 mb-8">
          {(post.tags || []).map((tag, idx) => (
            <span key={idx} className="text-[11.5px] font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-none border border-blue-100">
        ...
        {post.imageUrl && !hasImageInContent && (
          <div className="w-full md:w-1/2 mb-10 border border-slate-50 overflow-hidden bg-slate-50 shrink-0">
            <img src={post.imageUrl} alt="Post Content" className="w-full h-auto object-contain max-h-[500px]" />
          </div>
        )}

        <div className="text-[14.5px] text-slate-700 mb-10 leading-[1.8] font-medium prose prose-slate max-w-none flex-1" dangerouslySetInnerHTML={{ __html: post.content }} />

        {post.linkUrl && (
          <div className="mb-10">
            <a href={post.linkUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2.5 px-6 py-3 bg-slate-900 text-white rounded-none font-black text-[12px] hover:bg-blue-600 transition-all tracking-wider">🔗 관련 링크 바로가기</a>
          </div>
        )}
        ...
        <div className="mt-auto pt-8 border-t border-slate-100">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="w-11 h-11 rounded-none bg-slate-50 overflow-hidden border border-slate-100 shrink-0 shadow-sm">
                <img src={`https://api.dicebear.com/7.x/adventurer/svg?seed=${post.author}`} alt="avatar" className="w-full h-full object-cover" />
              </div>
              <div className="flex flex-col">
                <span className="font-black text-[14px] text-slate-900 leading-none mb-1.5">{post.author}</span>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              <button onClick={() => onLikeClick?.(null, post.id)} className={`flex-1 md:flex-none flex items-center justify-center gap-2.5 px-6 py-2.5 rounded-none transition-all duration-300 border-2 ${isLikedByMe ? 'bg-rose-500 text-white border-rose-500' : 'bg-white text-rose-400 border-slate-200 hover:border-rose-400'}`}><svg className={`w-4 h-4 transition-colors ${isLikedByMe ? 'fill-current' : 'fill-none'}`} viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z" /></svg><span className="text-[13px] font-[1000]">{formatKoreanNumber(post.likes || 0)}</span></button>
