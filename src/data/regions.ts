// src/data/regions.ts — 한국 17개 시/도 + 시/군/구 정적 데이터
// 행정안전부 2024년 기준 행정구역

export interface SidoEntry {
  sido: string;           // "서울특별시"
  shortName: string;      // "서울" (UI 표시용)
  sigungu: string[];      // ["강남구", "강동구", ...]
}

export const REGIONS: SidoEntry[] = [
  {
    sido: "서울특별시",
    shortName: "서울",
...
// 헬퍼: 시/도 이름으로 시/군/구 목록 조회
export const getSigunguList = (sido: string): string[] => {
  const entry = REGIONS.find(r => r.sido === sido);
  return entry ? entry.sigungu : [];
};

// 헬퍼: 짧은 표기 ("서울 강남구")
export const formatRegion = (region: { sido: string; sigungu: string }): string => {
  const entry = REGIONS.find(r => r.sido === region.sido);
  const sidoShort = entry?.shortName ?? region.sido;
  return `${sidoShort} ${region.sigungu}`;
};
