...
### 6.1 홈 탭 (`activeTab`)
- **새글 (any)**: 등록 후 3시간 이내 모든 글.
- **등록글 (recent)**: 3시간 경과 + 좋아요 3개 이상 (새글 심사 통과 기준).
- **인기글 (best)**: 좋아요 10개 이상.
- **최고글 (rank)**: 좋아요 30개 이상.
...
- **미등록**: 3시간 경과 + 좋아요 3 미만. 홈 피드 어디에도 노출되지 않음. 내정보 나의 기록·나의 한컷에서만 접근 가능.

### 6.1.1 재등록 시스템
- **대상**: 미등록 글 (3시간 경과 + 좋아요 3 미만 + `repostedAt` 필드 없음)
- **동작**: 제목 앞 `[재등록]` 추가 + `createdAt` 리셋(새글 복귀) + `repostedAt` 타임스탬프 기록
- **제한**: 1회만 가능. 재등록 후 다시 3시간 내 좋아요 3개 미달 시 영구 미등록.
...
- **노출 조건**: 등록글 기준과 동일 — **3시간 경과 + 좋아요 3개 이상**.
