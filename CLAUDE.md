...
## 🤝 모델 역할 분담 (Advisor / Worker) — 최우선 작업 방식

> **목적**: 판단(Advisor)과 구현 노동(Worker)을 분리해 코딩 속도를 높이고 토큰 소모를 억제한다. 아래는 코드 작업 시의 **기본 운영 모드**이며, 「절대 수칙」과 충돌하면 이 항목의 경계 규칙을 따른다.

**Advisor (메인 세션) — 직접 한다**
- 요구사항 분석·작업 분해·설계 결정, Worker 작업 브리프 작성
- 결과 검증: **diff 직접 확인 + 테스트/빌드 직접 실행**
- 최종 커밋 승인, 사용자 보고
- 판단에 필요한 최소 탐색만 직접. **대형 파일 전문·서브에이전트 output 파일을 직접 통독하지 말 것**(결론만 수령).

**Worker (서브에이전트, `Agent` 도구 · `model: "opus"` 지정) — 위임한다**
- 코드 작성·수정, 테스트 작성 등 구현 작업 전부
- 광범위 코드 탐색·조사(결론만 필요할 때)는 `Explore`에 위임
- 서로 독립적인 작업은 **한 메시지에서 병렬 위임**

**브리프 작성 기준** (Worker가 재탐색하지 않도록):
- Advisor가 이미 파악한 컨텍스트(파일 경로·관련 함수·데이터 흐름)를 담는다
- 프로젝트 컨벤션, 알려진 함정, **완료 기준(통과할 테스트·빌드 조건)** 명시
- 아래 「절대 수칙」·「금지 사항」을 브리프에 그대로 상속시킨다(코드 보호·Surgical Edit·전체 덮어쓰기 금지·무단 리팩터링 금지)

**경계 규칙 (기존 수칙과의 관계)**:
- **수칙 2 「선 보고 후 실행」**: Advisor↔사용자 경계에 적용. Worker 위임 **전** 계획(AS-IS→TO-BE)을 사용자에게 보고·승인받고, Worker 결과도 Advisor가 검증한 뒤 보고한다. 위임 자체는 승인된 계획의 실행 단계다.
- **Worker 완료 보고를 그대로 믿지 않는다** — diff·테스트로 확인 후 승인. 검증 실패는 **수정 브리프로 재위임**(직접 수정은 사소한 마무리·한두 줄에 한정).
- **위임 오버헤드가 더 큰 작업**(한두 줄 수정, 단순 문서 편집)은 Advisor가 직접 처리한다.
- **`firebase deploy`·`git push`는 Worker에게 위임 금지** — 사용자 명시 승인 후 Advisor가 직접 실행(「금지 사항」 유지).
...
2. **선 보고 후 실행**: 코드 수정 전 AS-IS → TO-BE를 한국어로 설명하고 사용자 확인 후 실행. (Advisor/Worker 모드에서는 Advisor↔사용자 경계에 적용 — 「🤝 모델 역할 분담」 경계 규칙 참조)
...
| `advertiser/AdCampaignForm.tsx` | 📢 광고 등록/수정. v2 P0-2 빈도 캡 섹션 + P0-1 노출 추정 카드(debounce 500ms callable) + P1-7 estimateAdReach + P1-8 Brand Safety(blockedCategories default '유배·귀양지'). 17개 시·도 + 6개 빠른 선택 묶음. v3.1(2026-04-30) `adType: 'body' \| 'feed'` prop — 본문/피드 진입 분리 + isFeedAd 가드로 본문 전용 4섹션(imageStyle/imagePosition/targetCreatorId/슬롯top·middle·bottom) 숨김 + 헤더 종류 배지 + finalForm 강제 정리 + 미리보기 분기. |
| `advertiser/AdTypeSelector.tsx` | 📢 ADSMARKET v3.1 (2026-04-30) 광고 종류 선택 카드 — 새 광고 등록 진입 시 [📄 본문]/[📋 피드] 카드 2개. AdvertiserCenter creationStep='select' 단계에 노출 후 'form'으로 진입. 편집 모드는 건너뛰고 targetSlots 단독성으로 adType 자동 추론. |
