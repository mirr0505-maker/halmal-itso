...
// ════════════════════════════════════════════════════════════
// 🚀 ADSMARKET — 광고 경매 시장 Cloud Functions
// ════════════════════════════════════════════════════════════
...
// ============================================================
// 🚀 경매 엔진 — POST /adAuction
// 입력: { slotPosition, postCategory, postId, postAuthorId, postAuthorLevel }
// 출력: { success, ad, fallback }
// ============================================================
    ...
    try {
      ...
      return res.json({
        ...
        ad: {
          ...
          landingUrl: winner.landingUrl,
          ctaText: winner.ctaText,
          bidType: winner.bidType,
          chargeAmount: secondPrice,
        },
        fallback: null,
      });
    } catch (err) {
...
// ============================================================
// 🚀 일일 수익 집계 — 매일 00:05 KST
// adEvents → dailyAdRevenue → users.pendingRevenue
// ============================================================
exports.aggregateDailyRevenue = onSchedule(
  {
    schedule: "every day 15:05", // UTC 15:05 = KST 00:05
    region: "asia-northeast3",
    timeoutSeconds: 120,
    memory: "256MiB",
  },
  async () => {
    ...
    // 어제 날짜의 유효 이벤트 조회 (isSuspicious === false)
    ...
    for (const [authorId, events] of Object.entries(authorMap)) {
      ...
      // EXP 기반 레벨 계산 (프론트 calculateLevel과 동일)
      ...
      events.forEach(ev => {
        if (ev.eventType === "impression") {
          impressions++;
          if (ev.bidType === "cpm") gross += ev.bidAmount / 1000;
        } else if (ev.eventType === "click") {
          clicks++;
          if (ev.bidType === "cpc") gross += ev.bidAmount;
        }
      });
      ...
      await db.collection("dailyAdRevenue").doc(revId).set({
        ...
        postAuthorNickname: userData.nickname || "",
        postBreakdown: [],
        totalImpressions: impressions,
        totalClicks: clicks,
        grossRevenue: Math.floor(gross),
        creatorShare,
        platformShare: Math.floor(platformShare),
        revenueShareRate: creatorRate,
        creatorLevel: level,
        status: "provisional",
        createdAt: Timestamp.now(),
...
// ============================================================
// 🚀 부정행위 탐지 — 매일 00:00 KST
// adEvents 스캔 → isSuspicious 마킹
// ============================================================
exports.detectFraud = onSchedule(
  {
    schedule: "every day 15:00", // UTC 15:00 = KST 00:00
    region: "asia-northeast3",
    timeoutSeconds: 120,
    memory: "256MiB",
  },
  async () => {
    ...
    eventsSnap.docs.forEach(docSnap => {
      ...
      if (suspicious && !ev.isSuspicious) {
        batch.update(docSnap.ref, { isSuspicious: true });
        flagged++;
      }
    });
...
// ============================================================
// 🚀 정산 처리 — 매주 월요일 09:00 KST
// settlements(pending) → 검수 → completed
// ============================================================
exports.processSettlements = onSchedule(
  {
    schedule: "every monday 00:00", // UTC 00:00 = KST 09:00
    region: "asia-northeast3",
    timeoutSeconds: 120,
    memory: "256MiB",
  },
  async () => {
    ...
    const pendingSnap = await db.collection("settlements")
      .where("status", "==", "pending")
      .get();

    if (pendingSnap.empty) {
      console.log("[정산] 대기 건 없음");
      return;
    }
    ...
    for (const docSnap of pendingSnap.docs) {
      ...
      // 상태 업데이트: pending → processing → completed
      // Phase 4 실제 이체: 여기서 은행 API 호출 (현재는 자동 승인)
